 ...
 *
 * <p>
 *     这个eureka server启动的时候，会尝试从其他的eureka server上去抓取注册表的信息，
 *     如果抓取失败了，那么就不会让其他的服务实例来自己这里进行服务发现，获取自己的注册表的信息
 *
 ...
 * <p>
 *
 *  如果说当eureka server获取心跳的比例低于一定的比例的话，在一定的时间内，比如一共有20个服务实例，
 *  在15分钟之内，就10个服务实例来发送了心跳，eureka server就自动的认为自己出了网络故障，
 *  不会将那10个服务实例给摘除，而是让自己进入一个自我保护机制。就是自己不再摘除
